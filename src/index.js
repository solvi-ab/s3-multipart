...
export default class S3Multipart {
  constructor (options) {
    ...
    if (!this.createUpload || !this.getPartUrl || !this.completeUpload) {
      throw new Error('Incomplete options, missing createUpload/getPartUrl/completeUpload')
    }
  }
      ...
      return new Promise((resolve, reject) => {
        const nextPart = () => {
          if (partNumber >= totalParts) {
            ...
            const [xhr, promise] = this.sendPart(uploadId, file, currentPartNumber, onProgress)
            activeXhr.push(xhr)

            promise.then(etag => etags[currentPartNumber] = etag)
            .then(() => {
              completedParts++
  ...
  sendPart(uploadId, file, partNumber, onProgress) {
    ...
    return [request, this.getPartUrl(file, uploadId, partNumber, part.size)
    .then(partUrl => {
      return new Promise((resolve, reject) => {
        request.onreadystatechange = function () {
          if (request.readyState === 4) {
            if (request.status !== 200) {
              reject(new Error('Unexpected response HTTP ' + request.status + ' ' + request.statusText))
            } else {
